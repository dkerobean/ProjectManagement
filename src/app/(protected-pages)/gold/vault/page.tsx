...
import AddStockModal from '@/components/gold/AddStockModal';
...
export default function VaultPage() {
  ...
  const fetchInventory = useCallback(async () => {
    try {
      ...
      // We'll fetch all inventory for now to support both views
      // Ideally Audit Log would have its own pagination API
  ...
  // Derive Audit Log from Inventory (sorted by most recent update)
  const auditLog = [...inventory].sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
        ...
        <main className="flex-1 px-4 pb-28 overflow-y-auto space-y-5 pt-2">
          ...
          {activeTab === 'inventory' ? (
                        ...
                        {inventory.slice(0, 3).map((item) => (
                            <div key={item._id} className="flex items-center justify-between group">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="text-sm font-bold text-white">Stock Updated</div>
                                        <div className="text-[10px] text-gray-500">{new Date(item.updatedAt).toLocaleDateString()}</div>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className="text-sm font-bold text-accent-green">{formatWeight(item.weightGrams)}</div>
                                    <div className="text-[10px] text-gray-500">{item.purity}</div>
                                </div>
                            </div>
                        ))}
                         {inventory.length === 0 && (
                             <div className="text-center py-4 text-xs text-gray-600">No recent movements</div>
                         )}
                    </div>
                </div>
            </motion.div>
            ...
            >
                {/* Audit Log View */}
                {auditLog.map((item) => (
                    <div key={item._id} className="bg-[#1C1C1E] rounded-2xl p-4 border border-white/5">
                        <div className="flex justify-between items-start mb-2">
                             <div>
                                 <h3 className="text-sm font-bold text-white mb-0.5">{item.purity} {item.goldType}</h3>
                                 <div className="text-[10px] font-mono text-gray-500">Batch: {item.batchId}</div>
                             </div>
                             <span className="text-xs font-bold text-gray-400 bg-white/5 px-2 py-1 rounded-lg">
                                {getLocationInfo(item.location).label}
                             </span>
                        </div>
                        <div className="flex justify-between items-end border-t border-white/5 pt-3 mt-2">
                            <div>
                                <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-0.5">Value</div>
                                <div className="text-sm font-bold text-accent-green">{formatCurrency(item.totalCost)}</div>
                            </div>
                            <div className="text-right">
                                <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-0.5">Date</div>
                                <div className="text-xs font-bold text-gray-300">{new Date(item.createdAt).toLocaleDateString()}</div>
                            </div>
                        </div>
                    </div>
                ))}
